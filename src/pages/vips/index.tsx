...
import { Consolidators } from "../conso/network";
...
export default function NetworkConsolidator() {
  ...
  const handleAssignConsolidator = (data: any) => {
    const disciple = data.disciples;
    const disciple_id = data.disciples.id;
    router.push(
      `/consolidations?assignTo=${disciple_id}&name=${disciple.first_name} ${disciple.last_name}`
    );
  };
  ...
  return (
    <>
      ...
      <Layout activeRoute="vips">
        <div className="flex items-center gap-4 mb-7">
          <h1 className="text-4xl font-bold flex items-center gap-3">VIPs</h1>
          <button
            className="gap-1 flex text-sm items-center underline text-[#6474dc]"
            onClick={() => setShowCreateVipModal(true)}
          >
            Add
              ...
              return (
                ...
                >
                  ...
                  <td className="py-1 pl-2 rounded-r-xl ">
                    <button
                      id={`btn-${item.id}`}
                      onClick={() => handleAssignConsolidator(item)}
